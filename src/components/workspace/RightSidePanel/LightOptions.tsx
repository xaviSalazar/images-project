...
const LightOptions = () => {
  ...
  useClickAway<MouseEvent>(positivePromptRef, () => {
    if (positivePromptRef?.current) {
      const input = positivePromptRef.current as HTMLInputElement;
      input.blur();
    }
  });
  ...
  const renderNegativeLightPrompt = () => {
    return (
      <div className="flex flex-col gap-4">
        <LabelTitle
          text="EVITAR DIBUJAR EN LUZ (colores, formas, etc)"
          url="https://huggingface.co/docs/diffusers/main/en/using-diffusers/inpaint#negative-prompt"
          toolTip="Palabras negativas para que el modelo de generacion no dibuje esos aspectos"
        />
        <div className="pl-2 pr-4">
          <Textarea
            ...
            placeholder=""
            id="negative-prompt"
            value={settings.negativePhotoLighting}
            onInput={(evt: FormEvent<HTMLTextAreaElement>) => {
              evt.preventDefault();
              evt.stopPropagation();
              const target = evt.target as HTMLTextAreaElement;
              updateSettings({ negativePhotoLighting: target.value });
            }}
          />
        </div>
      </div>
  ...
  const renderPositiveLightPrompt = () => {
    return (
      <div className="flex flex-col gap-4">
        <LabelTitle
          text="TIPO DE LUZ (En ingles)"
          toolTip="Escribir tipo de luz. Ejemplo: moorning light"
        />
        <div className="pl-2 pr-4">
          <Textarea
            ...
            onInput={(evt: FormEvent<HTMLTextAreaElement>) => {
              ...
              evt.stopPropagation();
              const target = evt.target as HTMLTextAreaElement;
              updateSettings({ photoLighting: target.value });
            }}
          />
        </div>
      </div>
      ...
      <div className="flex flex-col gap-1">
        ...
        <RowContainer>
          <Slider
            ...
            max={1}
            step={0.01}
            value={[settings.minimumLight]}
            onValueChange={(vals) => updateSettings({ minimumLight: vals[0] })}
          />
          <NumberInput
            id="steps"
            className="w-[60px] rounded-full"
            numberValue={settings.minimumLight}
            allowFloat={true}
            onNumberValueChange={(val) => {
              updateSettings({ minimumLight: val });
            }}
          />
        </RowContainer>
      </div>
      ...
      <div className="flex flex-col gap-1">
        ...
        <RowContainer>
          ...
          <NumberInput
            id="guidance-scale"
            className="w-[60px] rounded-full"
            numberValue={settings.maximumLight}
            allowFloat={true}
            onNumberValueChange={(val) => {
              updateSettings({ maximumLight: val });
            }}
          />
        </RowContainer>
      </div>
